...
def main():
    args = parse_args()
    train_file = '/home/g21tka10/test/dataset/Biaffine' + args.dataset + '/train.json'
    test_file = '/home/g21tka10/test/dataset/Biaffine' + args.dataset + '/test.json'
...
def load_glove_vocab(filename, wv_dim):
    vocab = set()
    with open('/home/yanzehao/' + filename, encoding='utf8') as f:
        for line in f:
            elems = line.split()
...
def build_embedding(wv_file, vocab, wv_dim):
    ...
    with open('/home/yanzehao/' + wv_file, encoding="utf8") as f:
        for line in f:
            elems = line.split()
            token = ''.join(elems[0:-wv_dim])
            if token in w2id:
